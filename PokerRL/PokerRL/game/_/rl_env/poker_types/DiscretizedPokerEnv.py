...
class DiscretizedPokerEnv(_PokerEnv):
    ...
    def __init__(self,
        ...
        self.other_bet_sizes_list_as_frac_of_pot = sorted(env_args.other_bet_sizes_list_as_frac_of_pot)  # ascending
        ...
        if action_int == 1:
            ...
            if self.other_bet_sizes_list_as_frac_of_pot is not None:
                if self.n_raises_this_round == 0:
                    ...
                    selected = self.get_fraction_of_pot_raise(fraction=self.other_bet_sizes_list_as_frac_of_pot[action_int - 2],
                                                      player_that_bets=self.current_player)
            else:
                selected = self.get_fraction_of_pot_raise(fraction=self.bet_sizes_list_as_frac_of_pot[action_int - 2],
                                                      player_that_bets=self.current_player)
