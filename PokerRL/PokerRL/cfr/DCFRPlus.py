...
class DCFRPlus(_CFRBase):
    ...
    def _regret_formula_after_first_it(self, ev_all_actions, strat_ev, last_regrets):
        ...
        regrets = (
           np.maximum(
                regrets * np.power(T - 1, 1.5) / (np.power(T - 1, 1.5) + 1.5)
                + imm_regrets
                , 0
           )
        )
    ...
    def _add_strategy_to_average(self, p_id):
        def _fill(_node):
            if _node.p_id_acting_next == p_id:
                T = self._iter_counter + 1
                contrib = _node.strategy * np.expand_dims(_node.reach_probs[p_id], axis=1) * np.power(T, 3)
                if self._iter_counter > 0:
                    _node.data["avg_strat_sum"] = _node.data["avg_strat_sum"] * (T - 1) / T + contrib
                else:
